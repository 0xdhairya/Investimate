...
const removeEntity = (removedEntityNumber) => {
    ...
    entities = entities.filter((e) => !(e.text == text && e.file == file && e.category == category));

    entityListElement.removeChild(removedDiv);
    entitiesCount--;

    const remainingEntities = Array.from(entityListElement.children);
    console.log('HEREEEEEEEE', remainingEntities);
    remainingEntities.forEach((child, index) => {
        const newIndex = index + 1; // Adjust indices starting from 1
        ...
        if (removeButton) {
            ...
            removeButton.removeEventListener("click", () => removeEntity(removedEntityNumber)); // Remove old listener
            removeButton.addEventListener("click", () => removeEntity(newIndex)); // Add new listener
        }
    });
};
...
const entityItem = (annotations, i, hidden = false) => {
    ...
    valueSelect.addEventListener("change", () => {
        ...
        const text = selectedOption.value;
        const file = selectedOption.dataset.file;
        entities.push({ file, text, category });
